...
$result=mysql_query("create view view_s as select * from emplyee where SoftLock=1;");
$result=mysql_query("select * from view_s");
while ($row = mysql_fetch_array($result)) {
    $cbname="sl".$row['ID'];
if (isset($_POST[$cbname])) {
    mysql_query("update emplyee set SoftLock=2 where ID={$row['ID']};");
}
}
    ?>
